...
class BOMImporter
{
    ...
    public function stringToBOMEntries(Project|Assembly $importObject, string $data, array $options): array
    {
        ...
        return match ($options['type']) {
            self::IMPORT_TYPE_KICAD_PCB => $this->parseKiCADPCB($importObject, $data, $options)->getBomEntries(),
     ...
     * @param array             $options Additional options for custom parsing behavior (optional).
     *
     ...
     */
    private function parseKiCADPCB(Project|ASsembly $importObject, string $data, array $options = []): ImporterResult
    {
        $result = new ImporterResult();
